...
public class SangpumCrud {
	...
	public static void main(String[] args) {
		...
		/*
		 * // 자료 수정 try { transaction.begin(); SangpumTable sangtab =
		 * em.find(SangpumTable.class, "6"); if(sangtab == null) {
		 * System.out.println("해당 자료 없음"); transaction.rollback(); } else { String
		 * newName = "마스크"; sangtab.setSang(newName); transaction.commit(); } } catch
		 * (Exception e) { System.out.println("up err : " + e); transaction.rollback();
		 * return; }
		 */
		...
		try {
			...
			System.out.println("\n전체 자료 읽기 2");
			// TyperdQuery를 이용해 JPQL 사용
			/*
			 * TypedQuery<SangpumTable> queryq1 =
