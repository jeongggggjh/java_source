	...
	public Ex49Memojang() {
		...
		addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent e) {
				...
				int result = JOptionPane.showConfirmDialog(Ex49Memojang.this, "정말 종료 하시겠습니까 ?", "", JOptionPane.YES_NO_OPTION);
				switch (result) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == mnuNew) { 				// 새문서
			...
			try {
				...
				JOptionPane.showMessageDialog(this, e2.getMessage(), "에러", 
						JOptionPane.WARNING_MESSAGE);		// 사용자용
			}
		}
			...
			try {
				...
				JOptionPane.showMessageDialog(this, e2.getMessage(), "에러", 
						JOptionPane.WARNING_MESSAGE); 		// 사용자에게 보여줄 메세지
			}
		}
			...
			System.out.println("About 호출 후 상황"); 	// Modal, Modaless 
		}
		...
		else if(e.getSource() == m_white) { // 팝업 메뉴 용
			txtMemo.setBackground(Color.WHITE);
		}
		else if(e.getSource() == m_blue) { // 팝업 메뉴 용
			txtMemo.setBackground(Color.BLUE);
		}
		else if(e.getSource() == m_yellow) { // 팝업 메뉴 용
