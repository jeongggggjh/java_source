...
// Stream 인터페이스 : 컬렉션, 배열 등의 저장 요소를 하나씩 참조하며

// 인터페이스(람다식)을 적용하며, 반복 처리가 가능하게 한다. 반복자 역할.
...
public class MyStream {
	...
	private void test3() {
		...
		double avg = slist.stream() // stream 객체 생성
				.mapToInt(Student :: getAge) // Student 객체 age 값으로 매핑해 age로 새 스트림 생성
		...
		double avgM = slist.stream().filter(m -> m.getGender().equals("남"))
				...
				.average()
				.getAsDouble(); // OptionalDouble에 저장한 값 읽기
		System.out.println("남자 나이 평균 : " + avgM);
		
		// '마' 성을 가진 자료 출력
		slist.stream().filter(ma -> ma.getName().startsWith("마")).forEach(name -> System.out.println(name.getName()));
	}
